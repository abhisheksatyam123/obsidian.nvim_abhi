...
M.substitute_template_variables = function(text, client, note)
  ...
  if not methods["task_stats"] then
    methods["task_stats"] = function()
      local stats_mod = require "obsidian.tasks_stats"
      local today = os.date "%Y-%m-%d"
      local stats_result = nil
      
      stats_mod.get_daily_stats(tostring(client.dir), today, function(s)
        stats_result = s
      end, true)
      
      if stats_result then
        local report_lines = stats_mod.format_report(stats_result, today)
