...
--- Toggle a checkbox on the current line through an ordered list of states.
--- This is the basic toggle used by ObsidianToggleCheckbox (non-task-aware).
---@param checkboxes string[] ordered list of checkbox characters (e.g. {" ", "x"})
util.toggle_checkbox = function(checkboxes)
  ...
  local indent_str, state, rest = string.match(line, "^(%s*%- %[)(.)(%].*)")
  if not state then
    -- Try to convert a list item into a checkbox.
    local list_pre, list_rest = string.match(line, "^(%s*%- )(.*)")
    if list_pre then
      vim.api.nvim_buf_set_lines(0, lnum, lnum + 1, false, { list_pre .. "[ ] " .. list_rest })
    end
    return
  end

  -- Find the current state in the list and advance to the next.
  local current_idx = nil
  for i, c in ipairs(checkboxes) do
    if c == state then
      current_idx = i
      break
    end
  ...
  local next_state
  if current_idx then
    next_state = checkboxes[(current_idx % #checkboxes) + 1]
  else
    next_state = checkboxes[1]
  end

  local new_line = indent_str .. next_state .. rest
