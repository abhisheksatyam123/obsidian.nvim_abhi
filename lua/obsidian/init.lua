...
obsidian.setup = function(opts)
  ...
  vim.api.nvim_create_autocmd({ "BufWritePre" }, {
    ...
    callback = function(ev)
      ...
      -- Update task statistics if placeholder exists.
      if client.opts.tasks.enabled then
        require("obsidian.tasks_stats").refresh_placeholder_in_buffer(client, bufnr)
      end
  ...
  -- Auto-pause tasks on exit.
  vim.api.nvim_create_autocmd({ "VimLeavePre" }, {
    group = group,
    callback = function()
      if client.opts.tasks.enabled and client.opts.tasks.auto_pause_on_exit then
        require("obsidian.tasks").pause_all_active(tostring(client.dir), nil, "exit", nil, true)
      end
    end,
  })
